...
EXCEL_FILE  = r"D:\RAG PROJECT\claims_rules.xlsx"
TOP_K       = 5
...
def load_rules_from_excel():
    ...
    for _, row in df.iterrows():
        ...
        kb.append({
            ...
            "insurance_company":
                str(row["insurance_company"]),

            "plan_name":
                str(row["plan_name"]),

            "cpt_codes":
                str(row["cpt_codes"]).split("|"),
...
def retrieve_candidates_faiss(
    extracted: dict,
    k: int = TOP_K,
) -> list[tuple[dict, float]]:
    """
    Embed the structured query chunk and search FAISS.
    Returns [(kb_entry, similarity_score), ...] best-first.
    similarity = 1 / (1 + L2_distance)
    """
    k = min(k, len(KB))
    query_chunk = _build_query_chunk(extracted)
    print(f"\nStructured FAISS query:\n{query_chunk}")

    q_vec = embed_model.encode([query_chunk]).astype("float32")
    ...
    for idx, dist in zip(indices[0], distances[0]):
        if 0 <= idx < len(KB):
            sim = float(1 / (1 + dist))
            results.append((KB[idx], sim))

    print(f"\nFAISS top-{k} candidates:")
    for i, (entry, sim) in enumerate(results):
        print(f"  [{i+1}] sim={sim:.4f}  id={entry['id']}  "
              f"denial={entry['denial_code']}  cpts={entry['cpt_codes']}")
...
MATCHING RULES (apply strictly, field by field):
  - group             : If KB value is "All" -> always matches.
                        If KB value is specific AND extracted is null -> NO MATCH.
                        If KB value is specific AND extracted is not null -> must equal KB value (case-insensitive).
  ...
  - denial_code       : If KB value is "All" -> always matches.
                        If KB value is specific AND extracted is null -> NO MATCH.
                        If KB value is specific AND extracted is not null -> must equal KB value exactly.
  ...
  - cpt_codes         : If KB cpt_codes contains "All" -> always matches.
                        ...
                        Otherwise -> at least one extracted CPT must appear in KB cpt_codes list.
...
def _format_candidates(candidates: list[tuple[dict, float]]) -> str:
    ...
    for i, (entry, sim) in enumerate(candidates, 1):
        lines.append(
            f"[{i}] id={entry['id']}  sim={sim:.4f}\n"
            f"    group={entry['group']!r}\n"
            f"    practice={entry['practice']!r}\n"
            f"    insurance_company={entry['insurance_company']!r}\n"
            f"    plan_name={entry['plan_name']!r}\n"
            f"    cpt_codes={entry['cpt_codes']}\n"
            f"    denial_code={entry['denial_code']!r}\n"
            f"    remark_code={entry['remark_code']!r}\n"
...
def llm_match(
    ...
    """
    Pass extracted fields + FAISS candidates to LLM for strict field matching.
    Returns (answer_text, response_time_ms).
    """
    prompt = (
...
def ask(question: str) -> dict:
    ...
    # Step 2: Build structured query chunk → FAISS search
    candidates = retrieve_candidates(extracted)

    # Step 3: LLM matches extracted fields against FAISS candidates
    result, elapsed_ms = llm_match(extracted, candidates)

    return {
        "matched":          result.get("matched", False),
        "rule_id":          result.get("rule_id"),
        "action":           result.get("action", "Manual Review Required"),
        "confidence":       result.get("confidence", 0.0),
        "confidence_label": result.get("confidence_label", "Low — manual review recommended"),
        "what_matched":     result.get("what_matched", []),
        "reason":           result.get("reason", ""),
        "instruction":      result.get("instruction", "No matching rule found in the KB."),
        "extracted":        extracted,
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("  Claims Denial KB — RAG  (LLM-first pipeline)")
    print("  Step 1 : LLM field extraction   (NL → structured JSON)")
    print("  Step 2 : Structured FAISS query (JSON → chunk → vector)")
    ...
    while True:
        ...
        print(f"Rule matched     : {result.get('rule_id', 'None')}")
        print(f"\nWhat was checked :")
        for line in result.get("what_matched", []):
            print(f"  - {line}")
        print(f"\nWhy this rule    : {result['reason']}")
        print(f"\nFull instruction :\n{result['instruction']}")
        print("─" * 60)
