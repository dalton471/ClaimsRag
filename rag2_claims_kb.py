...
Pipeline:
  ...
  2. BM25 retrieval         -> searches KB using extracted fields, returns top-K candidates.
  2.5 BM25 on Codes sheet   -> finds top-K relevant code categories using extracted CPTs.
  3. LLM field matching     -> strict rule-based match against candidates.
...
print(f"BM25 index built for {len(KB)} rules")
...
def retrieve_code_candidates(extracted: dict, k: int = 2) -> list:
    """
    BM25 search on the Codes sheet using extracted CPT codes as the query.
    Returns top-k matching code categories.
    Only rows with score > 0 are returned (meaning an actual match was found).
    """
...
OUTPUT FORMAT:
  ...
  If a match is found:
  {
    ...
    "what_matched": [
      "<field name>: extracted value matched KB value>"
    ],
    "reason": "<one plain-English sentence explaining why this rule applies>",
    "instruction": "<exact instruction sentence from the description — verbatim>"
  }
...
def validate_instruction_against_input(
    ...
    """
    STEP 4: Validates matched rule instruction against the original input.
    Receives top-K code candidates from Codes BM25 instead of full sheet.
    """
    ...
    # Format top code candidates for the prompt
    if code_candidates:
...
def ask(question: str) -> dict:
    ...
    try:
        ...
        return {
            "matched":          False,
            "rule_id":          None,
            "action":           "Field extraction failed",
            "confidence":       0.0,
            "confidence_label": "Low — manual review recommended",
            "what_matched":     [],
            "reason":           str(exc),
    ...
    # Step 2: BM25 retrieval on Rules KB
    candidates = retrieve_candidates(extracted)

    # Step 2.5: BM25 retrieval on Codes sheet
    code_candidates = retrieve_code_candidates(extracted)

    # Step 3: LLM matching — picks best rule from candidates
    match_result, elapsed_ms = llm_match(extracted, candidates)

    # Step 4: Validation — passes top code candidates instead of full sheet
